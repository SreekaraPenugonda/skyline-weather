...
function App() {
  ...
  return (
    <div className={`app-shell ${compact ? 'compact' : ''} weather-${weather?.condition?.toLowerCase() || 'clear'}`}>
      ...
      <main className="dashboard">
        ...
        <section className="weather-panel" aria-live="polite">
          {weather ? <>
            ...
            {view !== 'map' && <div className="hourly-strip">{hourly.map((item) => <div key={item.time}><span>{formatHour(item.time, twentyFourHour)}</span><img src={`https://openweathermap.org/img/wn/${item.icon}.png`} alt="" /><strong>{temperature(item.temperature)}°</strong><small>{item.rain_chance}% rain</small></div>)}</div>}
